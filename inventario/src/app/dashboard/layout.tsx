import { Package2, Users, LayoutDashboard, Settings, LogOut, ClipboardList, BarChart3 } from "lucide-react";
    ...
    <div className="min-h-screen bg-background text-foreground flex font-sans overflow-hidden relative">
      ...
      <aside className="w-64 bg-white/5 border-r border-white/10 backdrop-blur-xl flex flex-col relative z-20">
        ...
        <nav className="flex-1 px-4 space-y-2 mt-4">
          ...
          <Link href="/dashboard/prestamos" className="flex items-center gap-3 px-4 py-3 text-zinc-400 hover:text-white hover:bg-white/5 rounded-xl transition-all">
            <ClipboardList className="w-5 h-5" />
            <span className="font-medium">Préstamos y Salidas</span>
          </Link>
          <Link href="/dashboard/reportes" className="flex items-center gap-3 px-4 py-3 text-zinc-400 hover:text-white hover:bg-white/5 rounded-xl transition-all">
            <BarChart3 className="w-5 h-5" />
            <span className="font-medium">Reportes y Auditoría</span>
          </Link>
